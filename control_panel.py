...
class ControlPanel(QWidget):
    ...
    def viewCam(self):
        ...
        frame = cv2.resize(frame, (750,425))
    ...
    def viewCam2(self):
        ...
        def getPrn():
            ...
            class_id = 0 # Labels for the given file
            prn = {} #Mapping btw id - name
            # Data Preparation
            for filename in os.listdir(dataset_path):
                if filename.endswith('.npy'):
                    #Create a mapping btw class_id and name
                    prn[class_id] = filename[:-4]
                    print("Loaded "+filename)
                    data_item = np.load(dataset_path+filename)
                    face_data.append(data_item)
            ...
            while(not self.button2_state):
                ...
                for face in faces:
                    ...
                    #Display on the screen the name and rectangle around it
                    pred_prn = prn[int(out)]
                    
                    cv2.putText(frame,pred_prn,(x,y-10),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2,cv2.LINE_AA)
